    ...
    def add_slime_arguments(parser):
        ...
        def add_train_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Whether to use EMA to save memory during training.",
            )
            parser.add_argument(
                "--ema-decay",
                type=float,
            ...
            parser.add_argument(
                ...
                type=int,
                default=1,
                help="Update EMA every N training steps. Default is 1 (every step).",
            )
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                "--progressive-prefix-start-ratio",
                type=float,
                default=0.0,
                help=(
                    "Starting prefix ratio for progressive prefix distillation. "
                    "Fraction of the teacher's response tokens to prepend to the student's prompt "
                    "at the start of training. Linearly decays to --progressive-prefix-end-ratio "
                    "over --progressive-prefix-steps rollout steps. Default 0.0 (disabled)."
                ),
            )
            parser.add_argument(
                "--progressive-prefix-end-ratio",
                type=float,
                default=0.0,
                help=(
                    "Ending prefix ratio for progressive prefix distillation. "
                    "Target prefix ratio reached at rollout step --progressive-prefix-steps. Default 0.0."
                ),
            )
            parser.add_argument(
                "--progressive-prefix-steps",
                type=int,
                default=300,
                help=(
                    "Number of rollout steps over which the prefix ratio linearly decays "
                    "from --progressive-prefix-start-ratio to --progressive-prefix-end-ratio. Default 300."
                ),
            )
            parser.add_argument(
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "The path(s) to the prompt data. Supports files, directories, and multiple datasets for mixture. "
                    "Format: path1 [weight1] path2 [weight2] ... "
                    "Each path can be a file (.jsonl or .parquet) or a directory (reads all data files recursively). "
                    "Supports row slicing with @[start:end] suffix, e.g., 'data.jsonl@[0:1000]'. "
                    "Examples: "
                    "  --prompt-data /path/to/data.jsonl "
                    "  --prompt-data /path/to/data_folder/ (reads all data files in folder) "
                    "  --prompt-data /path/to/data1.jsonl /path/to/data2.jsonl (equal weights) "
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    ...
                    "This is useful for custom loss function."
                ),
            )
            parser.add_argument(
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Whether to include the verifiable reward in the log.",
            )
            return parser
        ...
        def add_wandb_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Specify the W&B run ID to resume. When set, wandb will resume the specified run "
                    "instead of creating a new one. This is useful for continuing training from a checkpoint."
                ),
            )
            return parser
