...
def merge_arrow_files(input_dir: str, output: str, max_tokens: int | None = None):
    ...
    # convert to a format that load_dataset can use 
    if "messages" in merged.schema.names:
        idx = merged.schema.get_field_index("messages")
        messages_json = pa.array(
            [json.dumps(row.as_py()) for row in merged.column("messages")],
            type=pa.string(),
        )
        merged = merged.set_column(idx, "messages", messages_json)
